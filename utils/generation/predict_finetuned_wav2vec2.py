#!/usr/bin/env python3
"""
Generate ASR predictions using finetuned Wav2Vec2 model on CGN test dataset.
"""
...
from run_wav2vec2 import load_cgn_data
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate ASR predictions using finetuned Wav2Vec2 model on CGN test dataset")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to finetuned Wav2Vec2 model directory"
    )
...
def main():
    ...
    test_dataset = load_cgn_data(args.data_dir, "test").select(range(100))
