#!/usr/bin/env python
# coding=utf-8
"""Fine-tuning HuBERT model for CTC-based ASR using CGN dataset and custom BPE tokenizer"""
...
def get_parameter_groups(
    ...
    # 统计层数
    num_layers = len(model.hubert.encoder.layers)

    groups = []  # 会填充多个 {params, lr, weight_decay, name}

    # Build exclusion set for 'other' group so that feature_extractor remains excluded
    fe_excluded_ids = set()
    if freeze_feature_extractor and hasattr(model.hubert, "feature_extractor"):
        for p in model.hubert.feature_extractor.parameters():
            fe_excluded_ids.add(id(p))
    ...
    # 2) encoder 层做 LLRD
    for layer_idx, layer in enumerate(model.hubert.encoder.layers):
    ...
    for mod_name in ["pos_conv_embed", "layer_norm"]:
        if hasattr(model.hubert.encoder, mod_name):
            decay, no_decay = [], []
            module = getattr(model.hubert.encoder, mod_name)
    ...
    decay, no_decay = [], []
    for n, p in model.hubert.feature_projection.named_parameters():
        (no_decay if is_no_decay(n) else decay).append(p)
    if decay:
    ...
    if not freeze_feature_extractor:
        ...
        decay, no_decay = [], []
        for n, p in model.hubert.feature_extractor.named_parameters():
            (no_decay if is_no_decay(n) else decay).append(p)
        if decay:
    ...
    for n, p in model.named_parameters():
        if id(p) in grouped_ids or id(p) in fe_excluded_ids:
            continue
        (no_decay if is_no_decay(n) else decay).append(p)
    if decay:
...
class FreezeEncoderCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if self.encoder_frozen and state.global_step >= self.freeze_encoder_steps:
            ...
            for param in self.model.hubert.parameters():
                param.requires_grad = True
            # Keep CNN feature_extractor frozen if desired
            if hasattr(self.model.hubert, "feature_extractor"):
...
class CTCTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        Always creates full parameter groups with target learning rates.
        Updates are controlled by requires_grad, not learning rates.
        """
        if self.optimizer is None:
            if self.use_layerwise_lr_decay:
                # Check if we're in Stage A (encoder frozen)
                encoder_frozen = not any(p.requires_grad for p in self.model.hubert.encoder.parameters())
                
                # Always create full parameter groups with final target learning rates
                logger.info(f"Creating optimizer with all parameter groups (Stage {'A - encoder frozen' if encoder_frozen else 'B - full model'})")
...
def main():
    ...
    # Load feature extractor (using Wav2Vec2FeatureExtractor for HuBERT)
